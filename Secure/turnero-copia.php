...
$beneficioturno = $usuario['beneficioTurno'];
...
<body>
    ...
    <?php if ($beneficioturno > 0){ ?>
        <!-- Si ya saco un turno traemos el mensaje con los datos -->
        <?php
    ...
    <?php } else { ?>
        <!-- Si no saco turno: -->
        <?php
        // Traemos configuración de los turnos de la BD
        ...
        while ($inicioFecha <= $finFecha) {
            $consultaFecha = $inicioFecha->format('Y-m-d');

            $stmt = $pdo->prepare("SELECT COUNT(*) FROM turnos WHERE fecha = ?");
            $stmt->execute([$consultaFecha]);
            $cupoActual = $stmt->fetchColumn();

            if ($cupoActual < $cupoFecha) {
                // Guardamos la fecha en formato d-m-Y para mostrarla
